...
class ActionDetector(nn.Module):
    ...
    def forward(self, slow_video, fast_video, boxes, objects=None, extras={}, part_forward=-1):
        ...
        #result, detector_losses, detector_metrics = self.roi_heads(slow_features, fast_features, boxes, objects, extras, part_forward)
        result, person_pooled, object_pooled = self.roi_heads(slow_features, fast_features, boxes, objects, extras, part_forward)

        if self.training:
            return detector_losses, detector_metrics, result

        return result, person_pooled, object_pooled
