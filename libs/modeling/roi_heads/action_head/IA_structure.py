...
from libs.utils.IA_helper import has_memory, has_person, has_object
...
class InteractionBlock(nn.Module):
    ...
    def forward(self, person, others):
        ...
        temp = self.key(key)
        if not temp.is_contiguous():
            temp = temp.contiguous()
        key_batch = temp.view(n, self.max_others, self.dim_inner, t_others, h_others, w_others)
        temp = self.value(key)
        if not temp.is_contiguous():
            temp = temp.contiguous()
        value_batch = temp.view(n, self.max_others, self.dim_inner, t_others, h_others, w_others)
...
class SerialIAStructure(IAStructure):
    ...
    def _aggregate(self, person_boxes, query, person_key, object_key, mem_key):
        block_count = dict()
        for idx, block_type in enumerate(self.I_block_list):
            print('idx',idx,'block_type',block_type)
            block_count[block_type] = block_count.get(block_type, 0) + 1
            name = block_type + "_block_{}".format(block_count[block_type])
...
def make_ia_structure(cfg, dim_in):
    ...
    print(cfg.IA_STRUCTURE.STRUCTURE)
