...
class AVAVideoDataset(data.Dataset):
    ...
    def _read_video_data(self, dirname, timestamp):
        ...
        while len(right_frames)<right_span:
            video_dir = os.path.join(video_folder, str(cur_t))
            frames_num = right_span - len(right_frames)
            frames = av_read_frames(video_dir, frames_num = frames_num, is_right = True)
            if len(frames)==0:
        ...
        while len(left_frames)<left_span:
            ...
            frames = av_read_frames(video_dir, frames_num = frames_num, is_right = False)
            if len(frames)==0:
                raise RuntimeError("Video {} cannot be decoded.".format(video_path))
